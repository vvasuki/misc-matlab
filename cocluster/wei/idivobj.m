function objval = idivobj(Z, W, rci, cci, Zm, Zr, Zc, basis)

% [objval] = idivobj(Z, W, rci, cci, Zm, Zr, Zc, basis)
%
% Compute the objective using I divergence.
% 

% Copyright 2008 Wei Tang (wtang@cs.utexas.edu)
% $Id: idivobj.m,v 1.4 2008/05/04 23:04:25 wtang Exp $
...
switch basis
	...
	case 5
		...
		Zh = full(spdotdiv(sum(Z*C,1),sum(W*C,1)));
		clear Z R C;
		h = Zgh(rci(I)+nrc*(cci(J)-1)).*Zr(I)./Zg(rci(I)).*Zc(J)'./Zh(cci(J))';
	case 6
		Zgh = full(spdotdiv(R'*Z*C,R'*W*C));
...
objval = sum(nonzeros(W).*(z.*log(z)-z.*log(h)-z+h));
end % main function